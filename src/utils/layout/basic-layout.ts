...
export function buildBasicLayout({ width, height, count, topPadding }: BasicLayoutInput): BasicBarSlot[] {
  if (count <= 0) return [];
  ...
  const effectiveTopPadding = topPadding ?? TOP_PADDING;
  const maxHeight = Math.max(0, baseY - effectiveTopPadding);
